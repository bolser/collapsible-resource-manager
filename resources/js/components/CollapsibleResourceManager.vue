<template>
    ...
    <div v-if="!isEmpty || data.linkTo" :class="[ data.type, 'select-none' ]">
        ...
        <component v-if="data.label && isTopLevel" v-bind="topLevelLink"
            ...
            <div v-if="data.icon" class="sidebar__icon" v-html="data.icon"/>

            <svg v-else class="sidebar__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        ...
        <template v-if="isGroup && data.resources.length">
            ...
            <button type="button" class="sidebar__link sidebar__link--sub"
                v-if="data.label"
